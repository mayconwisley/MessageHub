...
O Message Hub é configurado inteiramente por variáveis de ambiente. Existem cinco arquivos de exemplo, cada um com um propósito diferente — **nunca versione os arquivos `.env` efetivos**, apenas os `*.example`.
...
| [`.env.vps.example`](../.env.vps.example)               | `docker-compose.vps.yml`                                       | VPS Linux com PostgreSQL, RabbitMQ e Nginx executando diretamente no host.                                            |
...
| `HTTP_BIND_ADDRESS` | Não             | `0.0.0.0`               | Endereço no qual o backend escuta. O perfil de VPS força `127.0.0.1`, pois usa a rede do host e deve aceitar tráfego somente do Nginx local.                          |
...
| `VITE_BASE_PATH` | Não         | Caminho-base público do frontend, com padrão `/` e sempre com barras inicial e final. Para `https://www.mcnwly.com.br/messagehub/`, use `/messagehub/`. Também é embutido no bundle em tempo de build. |
...
No perfil `docker-compose.vps.yml`, PostgreSQL, RabbitMQ e Nginx não são criados. Backend e migration usam `network_mode: host` para alcançar os serviços locais por `127.0.0.1`; o backend é explicitamente limitado ao mesmo endereço. Um container temporário copia o build do frontend para `FRONTEND_PUBLIC_DIR`, que deve ser servido pelo Nginx do host. Esse perfil é exclusivo para Linux.
