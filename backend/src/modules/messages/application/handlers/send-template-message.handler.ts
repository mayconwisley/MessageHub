...
import { APPLICATION_REPOSITORY, IApplicationRepository } from '@modules/applications/domain/repositories/application.repository.interface';
import { PhoneNumberNotFoundError } from '@modules/phone-numbers/domain/errors';
import { IPhoneNumberRepository, PHONE_NUMBER_REPOSITORY } from '@modules/phone-numbers/domain/repositories/phone-number.repository.interface';
import { TemplateStatus } from '@modules/templates/domain/enums/template-status.enum';
import { ITemplateRepository, TEMPLATE_REPOSITORY } from '@modules/templates/domain/repositories/template.repository.interface';
...
export class SendTemplateMessageHandler implements ICommandHandler<SendTemplateMessageCommand> {
  constructor(
    ...
    @Inject(WHATSAPP_ACCOUNT_REPOSITORY) private readonly whatsAppAccountRepository: IWhatsAppAccountRepository,
  ...
  async execute(command: SendTemplateMessageCommand): Promise<Result<MessageDto, InvalidMessageError | ApplicationNotFoundError | PhoneNumberNotFoundError | TemplateNotFoundError>> {
    ...
    if (command.idempotencyKey) {
      const existing = await this.messageRepository.findByIdempotencyKey(applicationId, command.idempotencyKey);
    ...
    const templates = command.template.id
      ? [await this.templateRepository.findByMetaId(application.tenantId, account.id, command.template.id)]
      : command.template.name
        ? await this.templateRepository.findByName(application.tenantId, account.id, command.template.name)
        : [];
    const approvedTemplates = templates.filter(
      (template): template is NonNullable<typeof template> => template?.status === TemplateStatus.APPROVED,
    );
    ...
    const messageResult = Message.createTemplate({
      ...
      parameters: command.parameters.length ? [{ component: 'body', values: command.parameters }] : [],
