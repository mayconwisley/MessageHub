...
import { SystemLogLevel } from '../../application/ports/system-log.repository.interface';
...
export class SystemLogsController {
  ...
  async list(
    ...
    const result = await this.mediator.query(
      new ListSystemLogsQuery(query.page, query.pageSize, query.level, query.search),
    );
    return {
      ...result,
      items: result.items.map((log) => SystemLogResponseDto.fromDto(log)),
    };
  }
}
