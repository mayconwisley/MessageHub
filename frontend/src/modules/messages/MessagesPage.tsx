...
export function MessagesPage() {
  ...
  const send = useMutation({
    mutationFn: (data: FormData) =>
      messagesApi.send({ ...data, applicationId }),
    onSuccess: () => {
      client.invalidateQueries({ queryKey: ["messages", applicationId] });
      setFeedback({
        severity: "success",
        message:
          "Mensagem enfileirada para envio. Acompanhe o status na lista abaixo.",
      });
    },
    onError: (error) => {
      const requestId =
        error instanceof ApiError && error.requestId
          ? ` Protocolo: ${error.requestId}.`
          : "";
      setFeedback({
        severity: "error",
  ...
  useEffect(() => {
    if (
      validTenantId &&
      !applicationId &&
      applications.data?.items.length === 1
    ) {
    ...
    <Stack spacing={3}>
      ...
      {health.error && (
        <Alert severity="warning">
          A fila RabbitMQ está indisponível. As mensagens continuarão como
          pendentes até a conexão ser restabelecida. Consulte{" "}
          <code>/health</code> para o diagnóstico técnico.
        </Alert>
      )}
      <Stack spacing={2}>
          ...
          <PaginatedTable<Message>
            columns={[
              ...
              },
              {
                ...
                render: (row) => (
                  <Chip
                    label={toPresentationValue("status", row.status)}
                    size="small"
                  />
                ),
              },
      ...
      <FormDialog open={sendOpen} onClose={closeSend} title="Enviar mensagem">
        <Stack spacing={2} sx={{ mt: 1 }}>
          {send.isSuccess ? (
            ...
            >
              {send.error && (
                <Alert severity="error">{send.error.message}</Alert>
              )}
              <Controller
                name="phoneNumberId"
