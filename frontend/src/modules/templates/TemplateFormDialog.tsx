...
import { templateFormSchema, type TemplateFormData } from './template-form.schema';
...
export function TemplateFormDialog({
  ...
  return (
    <Dialog open={open} onClose={isSubmitting ? undefined : onClose} fullWidth maxWidth="lg">
      <DialogTitle>
        {isEditing ? `Editar modelo “${template?.name}”` : 'Novo modelo de mensagem'}
      </DialogTitle>
      <DialogContent dividers>
        <Stack direction={{ xs: 'column', lg: 'row' }} spacing={3} sx={{ pt: 1 }}>
          <Stack
            component="form"
            id="template-form"
            spacing={2}
          ...
          >
            ...
            {isEditing && template?.id && (
              <Alert severity="info">
                Alterações em um modelo publicado serão reenviadas à Meta para nova análise.
              </Alert>
            )}
            <Controller
              name="tenantId"
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <TextField
                ...
                helperText={form.formState.errors.name?.message ?? 'Ex.: confirmacao_pedido'}
              />
              <TextField
                fullWidth
                label="Idioma"
                ...
                helperText={form.formState.errors.language?.message ?? 'Ex.: pt_BR'}
              />
              <Controller
                name="category"
                control={form.control}
      ...
      <DialogActions>
        ...
        <Button type="submit" form="template-form" variant="contained" disabled={isSubmitting}>
          {isSubmitting ? 'Salvando...' : isEditing ? 'Atualizar modelo' : 'Criar modelo'}
        </Button>
      </DialogActions>
    </Dialog>
  );
