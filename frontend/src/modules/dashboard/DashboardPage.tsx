    ...
    <DashboardCard
      ...
      action={
        <Tooltip title="Atualizar agora">
          <IconButton
            aria-label="Atualizar estado da plataforma"
            onClick={() => void query.refetch()}
          >
            <RefreshOutlined />
          </IconButton>
...
function DeliveryQualityCard() {
  ...
  return (
    <DashboardCard
      ...
      subtitle="Mensagens finalizadas no período recente"
      minHeight={190}
    >
      <QueryContent query={query} skeletonHeight={112}>
        {(data: OperationalHealth) => (
...
function RecentMessagesCard() {
  ...
  return (
    <DashboardCard
      ...
      action={
        <Button size="small" endIcon={<OpenInNewOutlined />} onClick={() => navigate('/messages')}>
          Mensagens
        </Button>
      }
    >
...
function ResourceSummaryCard() {
  ...
  return (
    <DashboardCard
      ...
      subtitle="Recursos registrados na plataforma"
      minHeight={164}
    >
...
function QuickActions() {
  ...
  const actions = [
    {
      label: 'Consultar mensagens',
      description: 'Status, tentativas e erros do provedor',
      icon: <RuleOutlined />,
      path: '/messages',
    },
    {
      label: 'Gerenciar aplicações',
      description: 'Chaves, callbacks e números vinculados',
      icon: <InboxOutlined />,
      path: '/applications',
    },
    {
      label: 'Documentação da API',
      description: 'Contratos e exemplos de integração',
      icon: <ApiOutlined />,
      path: '/api-docs',
    },
    {
      label: 'Monitor de integrações',
      description: 'Quotas, credenciais, números e entrega',
      icon: <MonitorHeartOutlined />,
      path: '/monitoring',
    },
    {
      label: 'Alertas de engenharia',
      description: 'DLQs e falhas persistentes',
      icon: <WarningAmberOutlined />,
      path: '/engineering-alerts',
    },
    {
      label: 'Ambiente sandbox',
      description: 'Simulador de provider e webhooks',
      icon: <RuleOutlined />,
      path: '/sandbox',
    },
  ];
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
      {actions.map((action) => (
        <CardActionArea
          ...
          sx={{ flex: 1, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}
        >
        ...
        >
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Indicadores técnicos consolidados e atualizados automaticamente.
              </Typography>
            </Box>
          </Stack>
          <Chip
            icon={<ScheduleOutlined />}
            label="Atualização a cada 30 s"
            color="primary"
            variant="outlined"
          />
        </Stack>
