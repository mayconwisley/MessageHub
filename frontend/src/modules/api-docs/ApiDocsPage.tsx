...
const messageEndpoints: Endpoint[] = [
  ...
  },
  {
    ...
    curl: buildCurl(
      "GET",
      "/v1/messages/6f1c2e6a-2222-4b3a-9a11-3d0a4f0a1234/attempts",
    ),
  },
  {
    method: "GET",
...
const templateEndpoints: Endpoint[] = [
  ...
  },
  {
    ...
    curl: buildCurl(
      "GET",
      "/v1/templates/33333333-3333-3333-3333-333333333333",
    ),
  },
  {
    method: "PUT",
...
export function ApiDocsPage() {
        ...
        <CardContent>
          <Stack spacing={1.5}>
            ...
            <Typography color="text.secondary">
              Todas as chamadas abaixo usam uma API key de aplicação (não a
              sessão administrativa). Gere ou copie a chave em{" "}
              <Link href="/api-keys">Chaves de API</Link> e informe-a no
              cabeçalho{" "}
              <Typography
                component="code"
                sx={{
            ...
            <Alert severity="warning">
              A chave em texto puro só é exibida uma vez, no momento da criação.
              Guarde-a com segurança — o Hub armazena apenas o hash.
            </Alert>
            <Button
              component={Link}
              href={`${baseUrl}/docs`}
      ...
      <Card variant="outlined">
        <CardContent>
          <Stack spacing={2}>
            ...
            <Typography color="text.secondary">
              Use uma chave por aplicação, guarde-a em um cofre de segredos e envie uma chave de idempotência a cada operação de escrita.
            </Typography>
            <Typography variant="subtitle2">Node.js (fetch)</Typography>
            <CodeBlock code={`await fetch("${baseUrl}/v1/messages", {
  method: "POST",
  headers: {
  ...
  body: JSON.stringify({ phoneNumberId, to: "+5511999999999", content: "Olá" }),
            ...
            <CodeBlock code={`using var request = new HttpRequestMessage(HttpMethod.Post, "${baseUrl}/v1/messages");
